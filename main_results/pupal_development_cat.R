...
pupal_development_time_model <- lmer(pupa_development_time ~ pupa_temperature_treatment + 
                                       (1 | clutch_id), 
...
## remove male with deformed wings 

# filter 
adult_moths_minus_m1.cold <- adult_moths_minus_control %>%
  filter(adult_id != "m1.cold")

# remodel
pupal_development_time_outlier_model <- lmer(pupa_development_time ~ pupa_temperature_treatment + 
                                       (1 | clutch_id), 
                                     data = adult_moths_minus_m1.cold)
summary(pupal_development_time_outlier_model)
#### CHECK FIT ####

# compare the full model to a null model 
full_model <- lm(pupa_development_time ~ pupa_temperature_treatment, 
                 data = adult_moths_minus_control)
